...
export async function GET(request: Request) {
  try {
    ...
    // Call the backend API to get images
    const response = await axiosInstance.get('/api/image/get-images-by-user', {
      params: {
        email: email
      }
    });
    ...
    return NextResponse.json(
      { 
        ...
        message: "Operation failed",
        result: null,
        errors: [error instanceof Error ? error.message : "Error fetching images"]
      },
      { status: 200 }
    );
  }
